...
def compute_loss(target, target_p, predict, criterion, loss_mask):
...
try:
    with open(os.path.join(train_config["basepath"], "model.safetensors.index.json"), "r") as f:
        index_json = json.loads(f.read())
        head_path = index_json["weight_map"]["lm_head.weight"]
    with safe_open(os.path.join(train_config["basepath"], head_path),
                   framework="pt",
                   device="cpu") as f:
    ...
    with open(os.path.join(train_config["basepath"], "pytorch_model.bin.index.json"), "r") as f:
        index_json = json.loads(f.read())
        head_path = index_json["weight_map"]["lm_head.weight"]
    weights = torch.load(os.path.join(train_config["basepath"], head_path))
